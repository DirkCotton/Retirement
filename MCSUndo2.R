...
rage <- 65
portfolio <- 1000000
startwd <- 3 # lowest withdrawal %
maxwd <- 6 # highest withdrawal %
wdinc <- .5 # withdrawal steps %
...
hhline <- c("red4","dodgerblue4","darkgreen")
hhband <- c("mistyrose","slategray1","palegreen")
for(k in seq(3,6,.5)) {
  wr <- k/100
  failures <- 0
  events <- rep(n,0)
