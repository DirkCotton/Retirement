...
#####  Couple life expectancies are wrong and cause function le to fail
...
for(k in seq(wrstart, wrend, wrstep)) {
  ...
  print(paste("Hazard Rate= ",round(log(2)/median(lives),4),sep=" "))
  print(paste("***********************",sep=" "))
  ...
  survplot(objNpsurv,xlab="Year of Retirement",ylab="Probability of Portfolio Ruin",conf.int=.7,col.fill=hhband[household],col=hhline[household],grid=TRUE,main="XXX")
  title(main=plottitle)
}

myhist <- hist(lives)
hista <- hist(lives, breaks=seq(66,115,l=150))
points(socmd[2:50,1],socmd[2:50,5]*10)
